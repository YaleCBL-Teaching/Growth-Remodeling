...
|---|---|---|
| `video01_kinematic_growth.mp4` | fig01 | kinematic growth (single material) under hypertension: wall stress $\sigma/\sigma_h$ returns to homeostatic as the wall thickens |
| `video02_constrained_mixture.mp4` | fig02 | full-CMM turnover under hypertension: collagen & SMC stresses remodel back to $\sigma_h^k$ (and grow in mass); elastin stress stays elevated |
| `video03_compare_hypertension.mp4` | fig03 (top) | four theories adapting to hypertension (mass, mid-wall radius, thickness), converging onto the equilibrated target |
| `video03_compare_aneurysm.mp4` | fig03 (bottom) / fig04 | the three **constrained-mixture** theories under a mild aneurysm (elastin → 30%) — kinematic growth is dropped (an elastin-loss insult is meaningless for a single material); mass drops then rebuilds, settling onto the equilibrated line |
| `video04_stable_aneurysm.mp4` | — | full CMM, mild aneurysm (elastin → 30%): a **stable** adaptation — collagen & SMC remodel back, elastin stress plateaus, geometry settles to a new equilibrium |
| `video05_runaway_aneurysm.mp4` | fig05 (unstable) | full CMM, severe elastin loss (→ 3%): elastin stress runs away without bound — no equilibrium, the mechanistic signature of the instability |

The stable (`video04`) and runaway (`video05`) videos share **identical y-axes**
...
Non-time figures (fig00 setup, fig00 constitutive laws, and the parameter-sweep
panels of fig04/fig05) have no "live" analogue and stay as figures.
