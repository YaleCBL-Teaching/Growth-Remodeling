...
$$E = \tfrac{1}{2}\negthinspace\left(\lambda^2 - 1\right) = \tfrac{1}{2}(C-1),\qquad C = \lambda^2,$$

where $C = \mathbf{F}^{\negthinspace\top}\mathbf{F}$ is the right Cauchy–Green deformation
...
| Cauchy (true) | $\sigma$ | current config | $\sigma=\lambda_e^2\thinspace S$ (force / current area; $J=1$) |
...
$$\sigma_\theta = \frac{P\thinspace r}{h}\qquad\text{(circumferential wall stress, current config)},$$
...
(equivalently $S=\tfrac{1}{\lambda_e}\thinspace\mathrm{d}W/\mathrm{d}\lambda_e$). We use
...
$$W_e = \tfrac{c_e}{2}(I_1 - 3),\qquad
  S_e(\lambda_e) = c_e\negthinspace\left(1 - \lambda_e^{-3}\right).$$

**Collagen & smooth muscle — Fung exponential fiber** (crimped, then stiffens
steeply once recruited), $I_4 = \lambda_e^2$:

$$W = \frac{c_1}{4c_2}\Big(e^{\thinspace c_2 (I_4-1)^2} - 1\Big),\qquad
  S(\lambda_e) = c_1\thinspace (\lambda_e^2-1)\thinspace e^{\thinspace c_2(\lambda_e^2-1)^2}.$$

The Cauchy stress used in the spatial balance is then the push-forward
$\sigma = \lambda_e^2\thinspace S$ — for elastin
...
$(E_e(G^k),\thinspace S^k(G^k))$ marked. Elastin is soft; collagen and muscle stiffen
...
$$\boxed{\medspace\sigma_h^k := \sigma^k(G^k) = G_k^2\thinspace S^k(G^k)\medspace}\tag{2.1}$$
...
$$\lambda = \lambda_e \thinspace\lambda_{\text{inel}}.\tag{2.2}$$
