...
\begin{frame}[t]{Every Cohort Shares One Stretch}
  ...
  \slboxeq{2.2em}{eq:cohort}{\lambda_{n(\tau)}^{\alpha}(s,\tau) = \frac{\lambda(s)}{\lambda(\tau)} \, G^{\alpha}}
  The shared $\lambda(s)$ is the \emph{constraint}. At birth ($t=\tau$) the cohort
...
\begin{frame}[t]{Mass Balance}
  \textbf{Two G\&R times:} the \emph{current} time $s$ and a past
  \emph{deposition} time $\tau\in[0,s]$. Track each constituent's
  \textbf{apparent mass density} $\rho^{\alpha}$ (mass of $\alpha$ per current
  ...
  $\bar m^{\alpha}=\underbrace{m^{\alpha}}_{\text{production }\ge 0}
   -\underbrace{r^{\alpha}}_{\text{removal }\ge 0}$: \;
   $<0$ atrophy \;\textbullet\; $=0$ maintenance \;\textbullet\; $>0$ growth.
  \srccite{Humphrey, J Elasticity (2021)}
\end{frame}
...
\begin{frame}[t]{From Balance to a Heredity Integral}
  ...
  mixture, $\bm{v}^{\alpha}=\bm{v}$; as G\&R is slow vs.\ the cardiac cycle the
  transport term is negligible, so $\;\partial\rho^{\alpha}/\partial s=\bar m^{\alpha}\;$
  integrates directly in time.
  \par\smallskip
  ...
  \slboxeq{3.0em}{eq:massher}{\rho^{\alpha}(s)=
...
\begin{frame}[t]{Linear Momentum Balance}
  ...
  inertia $\rho\bm{a}$ is negligible --- the tissue stays in mechanical
  equilibrium throughout. G\&R is then a \emph{sequence} of finite-elasticity
  boundary-value problems.
  \srccite{Humphrey, J Elasticity (2021)}
\end{frame}

% ---- constitutive: Cauchy stress (4) ----------------------------------------
...
% ---- screenshot slide 2: the three constituent-specific constitutive relations
\begin{frame}[t]{Three Constituent-Specific Constitutive Relations}
  ...
  with wall shear:
  \slboxeq{2.8em}{eq:production}{m^{\alpha}(\tau)=m_o^{\alpha}\big(1
...
\begin{frame}[t]{Constituent Stored-Energy Functions}
  ...
  \slboxeq{3.8em}{eq:mixenergy}{\rho\,\tilde W^{\alpha}(s)=
...
% ---- two special cases from the paper: s=0 and homeostasis (6),(7) -----------
\begin{frame}[t]{Special Cases: $s=0$ and Homeostasis}
  ...
  \slboxeq{2.8em}{eq:homeo}{\rho\,\tilde W^{\alpha}(s)=
...
% ---- perfect maintenance (constant rates) from the presentation --------------
\begin{frame}[t]{Perfect Maintenance}
  ...
  \slboxeq{2.4em}{eq:perfmaint}{\text{perfect maintenance}\iff
