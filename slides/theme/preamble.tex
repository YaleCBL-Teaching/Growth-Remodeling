...
% ---- kinematic growth: F = F_e F_g -----------------------------------------
\newcommand{\kinmap}[1][1.0]{%
\begin{tikzpicture}[scale=#1,every node/.style={font=\small}]
  % reference (left)
  \draw[refbody] plot[smooth cycle,tension=0.9]
    coordinates {(0,0)(0.9,0.45)(1.5,0.1)(1.3,-0.8)(0.5,-0.95)(-0.1,-0.45)};
  \node[font=\footnotesize] at (0.65,-1.35) {reference $\mathcal{B}_0$};
  % grown, stress-free intermediate (middle, larger + shifted up)
  \begin{scope}[shift={(3.4,0.15)}]
    \draw[intbody] plot[smooth cycle,tension=0.9]
      coordinates {(0,0.1)(1.15,0.6)(1.9,0.15)(1.7,-1.0)(0.6,-1.2)(-0.15,-0.55)};
    \node[font=\footnotesize,align=center] at (0.8,-1.55) {grown, stress-free\\$\mathcal{B}_g$};
  \end{scope}
  % current, loaded (right)
  \begin{scope}[shift={(6.8,0)}]
    \draw[curbody] plot[smooth cycle,tension=0.9]
      coordinates {(0,0.05)(1.05,0.5)(1.75,0.1)(1.55,-0.9)(0.55,-1.05)(-0.1,-0.5)};
    \node[font=\footnotesize] at (0.7,-1.4) {current $\mathcal{B}_t$};
  \end{scope}
  % arrows (top: two-step split; bottom: composite, routed well below labels)
  \draw[cfgmaparrow] (1.6,0.15) to[bend left=18]
    node[above,font=\small]{$\bm{F}_g$ (growth)} (3.35,0.3);
  \draw[cfgmaparrow] (5.35,0.3) to[bend left=18]
    node[above,font=\small]{$\bm{F}_e$ (elastic)} (6.9,0.15);
  \draw[cfgmaparrow,black!55] (0.4,-2.35) to[bend right=10]
    node[below,font=\footnotesize,pos=0.5]{$\bm{F}=\bm{F}_e\,\bm{F}_g$} (7.6,-2.05);
\end{tikzpicture}}
